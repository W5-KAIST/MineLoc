...
if (!f.exists()) {
    ...
    for (var x = 0; x < width; x++) {
        for (var y = 0; y < height; y++) {
			...
			if (!upright) {
				if(data != 15){
					for(var z=0; z<replication_number; z++){
                sess.setBlock(origin.add(x, z, y), new BaseBlock(35, data));
					}
				}
			} else {
